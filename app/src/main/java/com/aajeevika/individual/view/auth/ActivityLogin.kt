...
import com.aajeevika.individual.databinding.ActivityLoginBinding
...
import com.aajeevika.individual.view.auth.viewmodel.LoginViewModel
    ...
    override fun observeData() {
        ...
        viewModel.userProfileLiveData.observe(this, {
            if(it.is_otp_verified == 0) {
                ...
                when {
                    it.user?.is_document_added == 0 -> {
                        val intent = Intent(this, ActivityDocuments::class.java)
                        intent.putExtra(Constant.USER_ROLE, preferencesHelper.roleId)
                        startActivity(intent)
                        finishAffinity()
                    }
                    it.user?.selectInterest == false -> {
                        val intent = Intent(this, ActivitySelectInterest::class.java)
                        startActivity(intent)
                        finishAffinity()
                    }
                    else -> {
